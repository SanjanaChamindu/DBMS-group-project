...
const AbsenceFunc = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("/leaves/getleaves");
        console.log(res.data);
        const leaves = res.data.map((element) => ({
          paygrade_id: element.paygrade_id,
          leave_type_name: "Just a leave",
          basic_salary: element.basic_salary,
          ot_benificts: element.ot_benefits,
          number_of_annual_leaves: element.number_of_annual_leaves,
          number_of_casual_leaves: element.number_of_casual_leaves,
          number_of_maternity_leaves: element.number_of_maternity_leaves,
          number_of_no_pay_leaves: element.number_of_no_pay_leaves,
        }));
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const leaves = state.leaves.filter(
          (e) => e.paygrade_id !== leave.paygrade_id
        );
        setState({ ...state, leaves });
  ...
  if (count === 0)
    return <p className="paragraph">Add new leave types to manage them</p>;
